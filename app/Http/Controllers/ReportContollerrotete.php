    ...
    public function drawDigarm($decks)
    {
        ...
        if (count($decks['checks']) > 0) {
            ...
            white-space: nowrap;z-index: 1;color:#BF0A30;font-size:10px;';
                ...
                $oddarray = [];
            $evengrp=0;
                $lineCss = 'position: absolute;background-color: #BF0A30;width:1px;transform-origin: 0% 0%;';
    ...
    transform-origin: 0% 0%;';
                ...
                foreach ($decks['checks'] as $key => $value) {
                    ...
                    if ($k % 2 == 1) {
                        ...
                        $oddarray[  $tooltipStart] =  $lineLeftPosition;
                        $rotationAngle = 10; // 10 degrees
...
$tooltipWidth = 31; // Assume a fixed width for the tooltip
$tooltipHeight = 34; // Assume a fixed height for the tooltip
$rotationAngle = 10;
// Convert the angle to radians
$angleRad = deg2rad($rotationAngle);

// Calculate the offset due to rotation
$leftOffset = ($tooltipWidth / 2) * (1 - cos($angleRad)) - ($tooltipHeight / 2) * sin($angleRad);
$topOffset = ($tooltipWidth / 2) * sin($angleRad) + ($tooltipHeight / 2) * (1 - cos($angleRad));

// Apply the offsets to the tooltip position
$adjustedTooltipLeft = $lineLeftPosition - 24 - ($lineLeftPosition * 10 / 100) - $leftOffset;
$adjustedTooltipTop = $tooltipStart - ($lineHeight * 5 / 100) - $topOffset;

                    $html .= '<div class="dot" style="top:' . $topshow . 'px; left:' . $leftshow . 'px; position: absolute;border: 2px solid #BF0A30;background: #BF0A30;border-radius: 50%;"></div>';

                    $html .= '<span class="line" style="position: absolute;background-color: #BF0A30;top:' . $lineTopPosition  . ';left:' . $lineLeftPosition . 'px;height:' . $lineHeight . 'px;' . $lineCss . ';transform: rotate('.$rotationAngle.'deg);"></span>';


                    $html .= '<span class="tooltip" style="' . $tooltipCss . 'top:' . $adjustedTooltipTop  . 'px; left:' . $adjustedTooltipLeft  . 'px;transform: rotate('.$rotationAngle.'deg);">' . $tooltipText . '</span>';
                }
                $html .= '</div>';
                $html .= '</div>';
                $html .= '</div>';
