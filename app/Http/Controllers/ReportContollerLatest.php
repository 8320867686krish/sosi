...
class ReportContoller extends Controller
{
        ...
        try {
            ...
            foreach ($ChecksList as $value) {
                if (count($value['checks']) > 0) {
                    ...
                    $vspPrepration = $this->drawDigarmWithTable($value, $deckI);
                    ...
                    for ($i = 1; $i <= $vspPreprationpageCount; $i++) {
                        ...
                        $templateId = $mpdf->importPage($i);
                        $mpdf->useTemplate($templateId, null, 20, null, null);
                        // Get the dimensions of the current page
    ...
    public function drawDigarmwithTable($decks, $pageWidth)
    {
        ...
        display: inline-block;">';
        if ($imageWidth > 1000) {
            $newImage = '<img src="' . $imageBase64 . '" id="imageDraw' . $pageWidth . '" style="width:100%" />';
        } else {
        ...
        foreach ($decks['checks'] as $key => $value) {
            ...
            if ($imageWidth > 1000) {
                $javascript = '<script>
                var newImageDiv = document.getElementById("imageDraw' . $pageWidth . '");
                var imageResizeWidth = newImageDiv.clientWidth;
                var mainWidth = "' . $imageWidth . '";
                var defineTop = "' . $top . '";
                var defineLeft = "' . $left . '";
                var scaleFactor1 = (imageResizeWidth * defineTop) / mainWidth;
                var scaleFactor2 = (imageResizeWidth * defineLeft) / mainWidth;

                var dot = document.createElement("div");

                dot.className = "dot";
                dot.style.top = (scaleFactor1) + "px"; // Fix: Removed quotes around defineTop
                dot.style.left = (scaleFactor2) + "px";;
                ...
                document.querySelector("#imgc' . $pageWidth . '").appendChild(dot);
            </script>';
            $html .= $javascript;
