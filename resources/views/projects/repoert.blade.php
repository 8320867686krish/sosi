<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
    <div class="accrodion-regular">
        @if ($project->ihm_table != 'IHM Gap Analysis')
            <div id="accordion4">
                <div class="card">
        ...
        @else
            <div class="card">
                <div class="card-body">
                    @php
                        $gapAnalysisData = json_decode(@$foundItems['gapAnalysis']['extraField'], true);
                    @endphp
                    <form novalidate method="post" id="gapAnalysisReportForm">
                        @csrf
                        <input type="hidden" name="project_id" id="project_id" value="{{ $project->id }}">
                        @if (isset($hazmats))
                            @foreach ($hazmats as $hazmat)
                                <div class="form-group">
                                    <label>{{$hazmat[0]['name']}}</label>
                                    <textarea name="gapAnalysis[{{$hazmat[0]['name']}}]" id="gapAnalysis_{{$hazmat[0]['id']}}" rows="3" class="form-control">{{ $gapAnalysisData[$hazmat[0]['name']] ?? '' }}</textarea>
                                </div>
                            @endforeach
                            <div class="form-group">
...
@push('js')
    <script>
        ...
        $('#gapAnalysisReportForm').submit(function(e) {
            handleFormSubmission.call(this, e, "{{route('addGapAnalysisReport')}}");
        });
    </script>
@endpush
