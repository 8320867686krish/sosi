        ...
        <div class="col-12 col-md-12 col-lg-12 cloneTableTypeDiv" id="cloneTableTypeDiv{{ $hazmat->hazmat_id }}">
            ...
            <div class="row">
                ...
                <div class="col-4 imagehazmat" id="imagehazmat{{$hazmat->hazmat_id}}">
                    ...
                    <div style="font-size: 13px">
                        @if (basename($hazmat->getOriginal('image')) != $hazmat->project_id)
                            {{ basename($hazmat->getOriginal('image'))}} <a href="{{ route('removeHazmatDocument', ['id'=>$hazmat->id, 'type'=>'image']) }}" class="text-danger font-size-20 float-right removeHazmatDocument">x</a>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-4 dochazmat" id="dochazmat{{$hazmat->hazmat_id}}">
                    ...
                    <div style="font-size: 13px">
                        @if (basename($hazmat->getOriginal('doc')) != $hazmat->project_id || !empty($hazmat->doc))
                            {{ basename($hazmat->getOriginal('doc'))}} <a href="{{ route('removeHazmatDocument', ['id'=>$hazmat->id, 'type'=>'doc']) }}" class="text-danger font-size-20 float-right removeHazmatDocument">x</a>
                        @endif
                    </div>
                </div>
            </div>
